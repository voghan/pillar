...
invoke when you attempt to `npm install` a native add-on.
...
in order to modify on current Windows. Therefore, run the following commands __inside a `cmd.exe` started with "Run as
Administrator"__:
